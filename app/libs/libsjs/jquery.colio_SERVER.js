			...
			(function(content_id) {
				$('body, html').stop().animate({scrollTop: position-90}, 500, function() {
					...
					(function(content_idd) {
						$('.colio-content,.colio-hidden').stop().animate({'opacity':0},300, function(content_id) {
							$('.colio-content:visible').hide();
							//$('#'+content_idd).animate({'opacity':1},300);
							...
							$('#'+content_idd).fadeIn('normal', function() {
								$('.colio-hidden').animate({'opacity':1},300);
							}).css('opacity',1);
							//alert(content_idd);
							//alert('Test');
						});
		...
		var _addContentDiv = function(content){
			...
			content.find('img').imagesLoaded(function() {
				...
				$('.custom_slider').flexslider({
				 ...
				 //$('.colio-content,.colio-hidden').animate({'opacity':1},300);
...
//			});
	...
	*/
	...
	Colio.prototype.expandViewport = function(content_id, adjust){
		...
		this.markup.stop().delay(300).animate({height: viewport_height}, duration, easing, $.proxy(function(){
...
//			}, this));
	...
	*/
	...
	Colio.prototype.collapse = function(){
		...
		$('.colio-content:visible,.colio-hidden').animate({'opacity':0},300);
	...
	$.fn.colio = function(c) {
		...
		var s = $.extend({
			...
			onExpand: function(content_id){
				//console.log(test);
				//alert(content_id);
				//$('.colio-hidden, #'+content_id).animate({'opacity':1},300);
				$('.colio-loader').fadeOut();
			...
			onCollapse: function(){
				$('.colio-hidden,.colio-content').css('opacity',0);
